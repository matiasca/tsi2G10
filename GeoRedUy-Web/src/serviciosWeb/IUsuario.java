...
//		System.out.println(respuesta);
	...
	public String listarContactosConectados(@FormParam("mail") String mail,
		...
		System.out.println("listarContactosConectados");
	...
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/mandarMensaje")
	public String mandarMensaje(@FormParam("mail") String mail,@FormParam("mensaje") String mensaje,
			@FormParam("token") String token,@FormParam("mailDestinatario") String mailDestinatario) throws JSONException {
		
		System.out.println("mandarMensaje");
		...
		try {
			...
			if ((mail != null) && (token != null))
				if (icusuario.usuarioValido(mail, token))
					if (icusuario.obtenerTokenCloud(mailDestinatario, token)!= null){
						Sender sender = new Sender(PuntoEntradaWS.getApikeycloud());
						Message message = new Message.Builder()
								.addData("mailEmisor", mail)
								.addData("mensaje", mensaje)
								.addData("tipo", "chat")
								.build();
		
						try {
							sender.send(message, icusuario.obtenerTokenCloud(mailDestinatario, token), 1);
						} catch (IOException e) {
	...
	public String listarContactosConectados(@FormParam("mail") String mail,
			@FormParam("token") String token,@FormParam("cadena") String mailInvitado) throws JSONException {

		
		System.out.println("enviarInvitacion");
		...
		try {
			...
			if ((mail != null) && (token != null) && (mailInvitado != null))
				if (icusuario.usuarioValido(mail, token))
					icusuario.enviarInvitacion(mail, mailInvitado);

			respuesta.put("mensaje", "Mail enviado con exito");
